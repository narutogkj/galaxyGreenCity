...
const TSMHospitalScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        setBigImage(restaurantData[1].menu[0].photo)
        setSeletedPlotName(restaurantData[1].menu[0].name)
        setSeletedPlotPrice(restaurantData[1].menu[0].price)
        setSeletedPlotDescription(restaurantData[1].menu[0].description)
    }, [])
    ...
    return (
        <ImageBackground source={images.bg} style={styles.container}>
            <View style={{ marginTop: 22 }}>
                <Header title="TSM Hospital" isHomeScreen navigation={navigation} />
            </View>
